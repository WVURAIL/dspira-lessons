...
![diagram]({{ site.baseurl }}/images/04/img/1.png)
...
We shall design FIR filters using the graphical tool that ships with GNU Radio. This tool requires an additional software dependency to work which can be installed by opening a new termina window and entering``sudo apt install python-qwt5-qt4``. Once the dependency is installed, in the taskbar (move mouse to the top of the window) click on ``tools`` and then ``filter design tool``

![filterdesign]({{ site.baseurl }}/images/04/img/fdt.png)
