...
summary:  A deep dive into Digital Signal Processing through a series of labs using GNU Radio
...
Now multiply this signal by the cosine and the sine "LO's".  This now will become your IQ signal.  In GNU Radio, this is passed into a "float to complex" block,
...
Use appropriate constant multiplies and adders to construct the above in GNU Radio, where \\(W^{i} = e^{\frac{-2\pi k i}{8}}\\). Are the outputs the valid frequency-domain results?
...
Where the sub-filter coefficients $$h(n + pN)$$ correspond to what are called P-tap 'polyphase sub-filters'. The N such polyphase sub-filters that make up this operation, together with the following DFT stage, are collectively called a 'polyphase filter bank' ('PFB'). A realization of this filter bank is shown below:
